...
export default function DiscoverT() {
  ...
  const getSortObject = (toFind, objArry) => {
    const { value, label } = objArry.find(({ value, label }) => {
  ...
  const onSortChange = ({ value, label }) => {
    ...
    // const { value, label } = getSortObject(val, sortOptions);
    setParams({ ...params, sortby: value });
    navigate(
      location.pathname +
        "?" +
        qs.stringify({ ...params, sortby: label.toLowerCase() }, qsOptions)
    );
  };
  ...
  const onApplyFilters = (filterState) => {
    ...
    const updatedParams = { ...filterState };
    setParams(updatedParams);
    setShowFilterMenu(false);
    const queryString = qs.stringify(updatedParams, qsOptions);
